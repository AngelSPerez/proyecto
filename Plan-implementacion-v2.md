...
Eres un desarrollador Flutter senior y arquitecto de software experto en aplicaciones multiplataforma modernas con Firebase. Tu tarea es crear una aplicación completamente funcional llamada RecipeRecive, un recetario digital moderno para cocina casera. La aplicación debe sentirse como una app profesional lista para Play Store o App Store, con una interfaz limpia, Material 3, soporte multilenguaje, autenticación real y arquitectura escalable. Debes generar código real, funcional y compilable, evitando pseudocódigo, placeholders innecesarios o implementaciones incompletas. Sigue estrictamente todas las instrucciones y no omitas ninguna funcionalidad importante.

La aplicación debe desarrollarse en Flutter usando Firebase Core, Firebase Authentication, Cloud Firestore y Firebase Storage. El proyecto debe llamarse recipe_recive y utilizar null safety con SDK >=3.0.0 <4.0.0. Configura correctamente pubspec.yaml incluyendo assets, dependencias y estructura de carpetas organizada en models, services, screens, widgets, utils y l10n. Implementa persistencia local usando SharedPreferences para guardar idioma y tema. El sistema de idiomas debe ser manual usando AppLocale.language y una clase Strings personalizada, sin usar el paquete intl. Los idiomas requeridos son inglés, español y portugués brasileño.

La aplicación debe incluir autenticación completa con registro, login, recuperación de sesión, cambio de contraseña, cambio de email y un sistema de administrador inicial automático. Implementa pantallas modernas y funcionales: SplashScreen, WelcomeScreen, LoginScreen, RegisterScreen, RecipeListScreen, RecipeDetailScreen, UserProfileScreen, ConfigurationScreen y un panel administrador completo. El panel admin debe permitir CRUD real para recetas, usuarios, comentarios, ratings, likes y recetas guardadas usando Firestore en tiempo real mediante streams.

La pantalla principal de recetas debe incluir búsqueda, favoritos, recomendaciones inteligentes, vista grid/lista, pull-to-refresh y navegación hacia detalles completos de cada receta. RecipeDetailScreen debe ser avanzada e incluir imagen principal, ingredientes, pasos, ratings interactivos, comentarios, compartir recetas, recetas similares y sistema de guardado. Implementa recomendaciones usando similitud de ingredientes con algoritmo Jaccard. Usa Navigator.push y pushReplacement para navegación, Material 3 con ColorScheme.fromSeed(Color(0xFF9C27B0)), soporte para tema claro/oscuro/sistema y transiciones FadeUpwardsPageTransitionsBuilder en todas las plataformas. Todos los errores asíncronos deben manejarse con SnackBars y las operaciones destructivas deben requerir confirmación del usuario. Genera todos los archivos completos, con imports correctos, widgets funcionales, lógica real y estructura profesional lista para ejecutarse.
